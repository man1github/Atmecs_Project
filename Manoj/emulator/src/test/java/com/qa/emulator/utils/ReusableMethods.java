...
public class ReusableMethods extends BaseTest {
	...
	public static void clearNotification() {
		try {
			List<WebElement> notificationList = driver.findElements(DragAndDropPage.NOTIFICATION_LIST);
			WebElement button = clickButton(DragAndDropPage.CLEAR_ALL);
	
			if (notificationList.size() >= 7) {
				swipeAction(DragAndDropPage.NOTIFICATION_LIST_4, "up");
				WebElement clearAllButton = clickButton(DragAndDropPage.CLEAR_ALL);
				if (clearAllButton.isDisplayed()) {
					waitForClickable(DragAndDropPage.CLEAR_ALL);
					logger.info("Clear all button is Clicked");
				}
			} else if (button != null) {
				waitForClickable(DragAndDropPage.CLEAR_ALL);
				logger.info("Clear all button is Clicked");
			} else {
				flingGesture(DragAndDropPage.NOTIFICATION_LIST_4, "down", "5000");
				logger.info("NO notification found, closed the notification");
			}
			logger.info("Notifications is Handled");
			test.log(Status.PASS, "Notifications is Handled");
		} catch (Exception exception) {
			logger.error("Notifications is Not Handled" + exception);
			test.log(Status.FAIL, "Exception Occured" + exception);
		}
	}
	...
	public JSONObject getJsonObject(String filePath, String key, String testCaseId) {
		try {
			FileInputStream inputStream = new FileInputStream(filePath);
			JSONTokener tokener = new JSONTokener(inputStream);
			JSONObject jsonObject = new JSONObject(tokener);
			object = jsonObject.getJSONObject(key).getJSONObject(testCaseId);
			return object;
...
/*
 * public static void screenOrientation(String modeType) {
try {
	...
	if (modeType.equalsIgnoreCase("LANDSCAPE")) {
		...
		test.log(Status.PASS, "LANDSCAPE - Screen Orientation is performed");
		...
		test.log(Status.PASS, "PORTRAIT - Screen Orientation is performed");
	}
	logger.info("Screen Orientation is performed");
	test.log(Status.PASS, "Screen Orientation is performed");
} catch (Exception exception) {
...
//			try {
//				FileUtils.copyFile(sourceScreenshot, new File(value));
//			} catch (Exception exceptions){
//				logger.error("Click is not performed : " + exceptions);
//			}
			logger.error("Click is not performed : " + exception);
			test.log(Status.FAIL, "Exception Occured" + exception);
		}
